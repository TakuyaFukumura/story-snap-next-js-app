...
- 画像を9:16（1080 × 1920px）のキャンバスに自動配置し、トリミング範囲を確定
- JPEG、PNG、WebPの読み込み（10 MB以下、4,000万画素以下）
- MediaPipeによる顔輪郭検出と輪郭に沿った自動ガウシアンぼかし（利用できない場合は矩形検出へフォールバック）
- キャンバス上のドラッグによる画像位置の調整
- 手動でモザイク範囲を追加
- 手動モザイク追加時のキャンバス拡大表示とスクロール
- モザイク対象範囲の個別選択
- モザイク対象範囲のドラッグによる位置調整
- ガウシアンぼかし／ブロックモザイクの切り替え
- モザイク強度（弱・中・強）の切り替え
- JPEGまたはPNGでの保存
...
1. 「画像を選択」から画像を読み込みます。
2. Step 1で画像をドラッグしてトリミング範囲を決め、「トリミングを確定」を押します。
3. Step 2で顔検出の成否と自動モザイク結果を確認し、「Step 3へ進む」を押します。
4. Step 3で必要に応じて手動範囲、モザイク方法、強度を設定します。手動追加中は表示倍率を変更して細かい範囲を選択できます。
